...
window.addEventListener('keydown', (e)=>{
  if(e.key === 'a' || e.key === 'A') attemptLane(-1);
  if(e.key === 'd' || e.key === 'D') attemptLane(1);
  if(e.key === 'w' || e.key === 'W') keyState.accel = true;
  if(e.key === 's' || e.key === 'S') keyState.brake = true;
  ensureAudioContext();
});
...
leftBtn.addEventListener('pointerdown', ()=>attemptLane(-1));
rightBtn.addEventListener('pointerdown', ()=>attemptLane(1));
accelBtn.addEventListener('pointerdown', ()=>keyState.accel=true); accelBtn.addEventListener('pointerup', ()=>keyState.accel=false); accelBtn.addEventListener('pointerleave', ()=>keyState.accel=false);
brakeBtn.addEventListener('pointerdown', ()=>keyState.brake=true); brakeBtn.addEventListener('pointerup', ()=>keyState.brake=false); brakeBtn.addEventListener('pointerleave', ()=>keyState.brake=false);
...
function animate(){
  ...
  const difficulty = Math.min(6.1, 1 + elapsed*0.05);
